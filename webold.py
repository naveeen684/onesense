...
def transcribe(audio):
    ...
    processed_text = transcriber({"sampling_rate": sr, "raw": y})["text"]
    ...
    generators = chat.generate_chat_reply_wrapper(processed_text, '', state)           
    for generator in generators:
        processed_text=generator
        print("Result", generator)
        soup = BeautifulSoup(html, 'html.parser')

        # Find all elements with the class "assistant-message"
        assistant_messages = soup.find_all(class_='assistant-message')

        # Extract the text from the first assistant message
        if assistant_messages:
            assistant_text = assistant_messages[0].find('p').text
